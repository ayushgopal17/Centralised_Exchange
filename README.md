...
- An in-memory bid/ask book and custom matching engine
...
flowchart LR
    ...
    B --> E[In-memory matching engine]
    E --> M[In-memory order book\nand balances]
...
6. The matching engine scans the opposite side of the in-memory order book:
   - a buy matches a sell priced at or below the buy limit;
   - a sell matches a buy priced at or above the sell limit.
7. Matching quantities are settled, order fill quantities/statuses are updated, and a fill is written to PostgreSQL.
...
cex-project/
├── backend/
│   ├── index.ts                 # REST API and in-memory exchange state
│   ├── engine.ts                # Limit-order matching and settlement
│   ├── middleware.ts            # JWT authentication middleware
│   ├── db.ts                    # Prisma client
│   ├── prisma/
│   │   ├── schema.prisma        # User, Stock, Order, and Fill models
...
| `GET` | `/depth/:symbol` | No | Return the local in-memory book |
...
- Balances and order books are held in process memory. Restarting the backend clears them, even though users, orders, and fills remain in PostgreSQL.
- A user loaded from an earlier database session will not automatically regain an in-memory balance after a backend restart.
- The executable market is currently limited to SOL/USD limit orders with whole-number quantities.
- The matching engine scans orders in insertion order; it is a prototype and does not yet maintain a fully sorted price-time-priority book.
...
- Persist balances and open order books transactionally
...
- Implement deterministic price-time priority and safer cancellation/refund logic
- Record both sides of each trade and expose a public trade tape
